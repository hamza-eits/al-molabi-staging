    ...
    <div class="row">
        <div class="col-12">
            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                <h3 class="mb-sm-0 font-size-18">{{ $title }}</h3>
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('voucher-content.index') }}",
                columns: [
            ...
            $('#create-update-form').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('voucher-content.store') }}",
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    cache: false,
        ...
        function editRecord(id) {
            var btn = $('#submit-btn');
            $.get("{{ route('voucher-content.edit', ':id') }}".replace(':id', id), function(response) {
        ...
        function deleteRecord(id) {
            if (confirm("Are you sure you want to delete?")) {
                $.ajax({
                    type: 'DELETE',
                    url: "{{ route('voucher-content.destroy', ':id') }}".replace(':id', id),
                    data: {
