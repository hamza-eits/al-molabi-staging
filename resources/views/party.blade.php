...
<select name="BranchID" class="form-select">
  @foreach($branches as $branch)
    <option value="{{ $branch->id }}" {{ (old('BranchID') == $branch->id) ? 'selected=selected' : '' }}>
      {{ $branch->name }}
    </option>
  @endforeach
</select>
...
<select name="PartyType" class="form-select">
  @foreach($party_type as $type)
    <option value="{{ $type->PartyCode }}" {{ (old('PartyType') == $type->PartyCode) ? 'selected=selected' : '' }}>
        ...
        <table class="table table-striped  table-sm  m-0" id="student_table">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td>{{$value->branch->name ?? 'N/A'}}</td> <!-- Ensure 'name' matches the branch model's attribute -->
                <td>{{$value->Address}}</td>
                <td>{{$value->Phone}}</td>
                <td>{{$value->Email}}</td>
                <td>{{$value->Active}}</td>
                <td>
                  <div class="d-flex gap-1">
                    <a href="{{URL('/PartiesEdit/'.$value->PartyID)}}" class="text-secondary">
                      <i class="mdi mdi-pencil font-size-15"></i>
                    </a>
                    <a href="#" class="text-secondary" onclick="delete_confirm2('PartiesDelete',{{$value->PartyID}})">
