                        ...
                        <form id="create-update-form" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12 mb-1">
                                    <label class="form-label">Shirka Name</label>
                                    <input type="text" name="shirka_name" id="shirka_name" class="form-control">
                                </div>
                                ...
                                <div class="col-md-3">
                                    ...
                                    <div id="logo_placeholder1"><img id="logo_placeholder" src="" class="img-fluid" style="max-width: 100%; margin-top:10px;" /></div>
                                </div>
    ...
    <script>
        ...
        $(document).ready(function() {
            table = $('#table').DataTable({
                processing: true,
                serverSide: true,
                ajax: "<?php echo e(route('shirka.index')); ?>",
                columns: [
            ...
            $('#create-update-form').on('submit', function(e) {
                ...
                $.ajax({
                    type: "POST",
                    url: "<?php echo e(route('shirka.store')); ?>",
                    dataType: 'json',
                    contentType: false,
                    processData: false,
                    cache: false,
        ...
        function editRecord(id) {
            var btn = $('#submit-btn');
            $.get("<?php echo e(route('shirka.edit', ':id')); ?>".replace(':id', id), function(response) {
                $('#record-id').val(response.data.id);
                $('#shirka_name').val(response.data.shirka_name);



             // Set images
                const basePath = "<?php echo e(asset('storage/')); ?>";
                // const basePath = "<?php echo e(asset('')); ?>";
                if (response.data.logo) {
                     $('#logo_placeholder').attr('src', `${basePath}/${response.data.logo}`);
                } else {
                    $('#logo_placeholder').attr('src', ''); // Or placeholder
                }
        ...
        function deleteRecord(id) {
            if (confirm("Are you sure you want to delete?")) {
                $.ajax({
                    type: 'DELETE',
                    url: "<?php echo e(route('shirka.destroy', ':id')); ?>".replace(':id', id),
                    data: {
...
<?php echo $__env->make('admin.layouts.admin', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u790884004/domains/xtbooks.cloud/public_html/rotana_sky/resources/views/shirka/index.blade.php ENDPATH**/ ?>